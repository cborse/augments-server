...
func (app *application) createSteamUser(steamID uint64) error {
	...
	for _, s := range *species {
		...
		// Insert creatures
		_, err := tx.Exec(
			"INSERT INTO creature (user_id, species_id, series_id, name, egg, staff_id, wins, action1, action2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			user.ID, s.ID, 1, s.Name, true, 0, neededWins, s.Type1, s.Type2)
		if err != nil {
			tx.Rollback()
			return err
		}
